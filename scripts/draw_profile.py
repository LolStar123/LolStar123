...
def svg(body, width, height, theme, title, motion=False):
    ...
    if motion:
        ...
        @media(prefers-reduced-motion:reduce){.push,.rock,.eyes{animation:none}}
        '''
...
def hero(theme, mobile=False):
    ...
    if mobile:
        ...
        return svg(body, 520, 468, theme, 'Atul Kanodia. Economics at UCL. A scribbled meowl pushes a boulder uphill.', True)
    ...
    return svg(art + body, 960, 370, theme, 'Atul Kanodia. Economics at UCL. A little further. A scribbled meowl pushes a boulder uphill.', True)
